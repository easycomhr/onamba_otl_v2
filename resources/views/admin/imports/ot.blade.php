...
@section('title', __('ui.nav.import_ot'))
@section('page_title', __('ui.nav.import_ot'))

@section('content')
<h1 class="text-2xl font-extrabold text-gray-900 uppercase tracking-tight mb-6 hidden md:block">{{ __('ui.nav.import_ot') }}</h1>

<div class="max-w-2xl mx-auto space-y-6">

    <div class="bg-blue-50 border border-blue-200 rounded-2xl p-5 flex items-start">
        <svg class="w-6 h-6 text-blue-600 flex-shrink-0 mr-3 mt-0.5" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" d="M11.25 11.25l.041-.02a.75.75 0 011.063.852l-.708 2.836a.75.75 0 001.063.853l.041-.021M21 12a9 9 0 11-18 0 9 9 0 0118 0zm-9-3.75h.008v.008H12V8.25z"/>
        </svg>
        <div>
            <p class="text-blue-800 font-semibold mb-1">{{ __('ui.import.guide_title') }}</p>
            <ul class="text-blue-700 text-[14px] space-y-1 list-disc list-inside">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <form method="POST" action="{{ route('admin.import.ot.store') }}" enctype="multipart/form-data" class="p-5 space-y-5">
            @csrf
            <div>
                ...
                <input type="file" name="file" accept=".xlsx,.xls,.csv" required
                       class="block w-full text-[15px] text-gray-800 border-2 border-dashed border-gray-300 rounded-xl px-4 py-6 cursor-pointer hover:border-blue-400 focus:outline-none focus:border-blue-500 transition-colors file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-bold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100">
                @error('file')
                    <p class="mt-2 text-sm font-medium text-red-600">{{ $message }}</p>
                @enderror
            </div>
            <div class="flex gap-3">
                <a href="{{ route('admin.import.ot.template') }}" class="flex items-center px-5 py-3 border-2 border-gray-300 text-gray-600 font-bold rounded-xl hover:bg-gray-50 transition-colors text-[15px]">
            ...
            <div class="p-5 space-y-5">
                <div class="grid gap-4 sm:grid-cols-2">
                    <div class="rounded-xl border border-green-200 bg-green-50 px-4 py-3">
                        <p class="text-sm font-semibold text-green-800">{{ __('ui.import.success_rows', ['count' => ($importResult['success'] ?? 0)]) }}</p>
                    </div>
                    <div class="rounded-xl border border-yellow-200 bg-yellow-50 px-4 py-3">
                        <p class="text-sm font-semibold text-yellow-800">{{ __('ui.import.skipped_rows', ['count' => ($importResult['skipped'] ?? 0)]) }}</p>
                    </div>
                </div>
